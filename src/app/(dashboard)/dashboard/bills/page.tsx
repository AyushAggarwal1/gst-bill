...
function BillsPageContent() {
  ...
  // Mint the public share link for a bill and copy it to the clipboard
  const handleCopyShareLink = async (billId: string) => {
    try {
      const res = await fetch(`/api/bills/${billId}/share`, { method: "POST" });
  ...
  // Firm name for share messages — fetched once, then cached
  const firmNameRef = useRef<string | null>(null);
  const getFirmName = async (): Promise<string> => {
    if (firmNameRef.current !== null) return firmNameRef.current;
    try {
      const res = await fetch("/api/profile");
      if (res.ok) {
        const data = await res.json();
        firmNameRef.current = data.firmName || "";
        return firmNameRef.current as string;
      }
    } catch {
      // fall through — message just omits the firm name
    }
    firmNameRef.current = "";
    return "";
  };

  // Share the public invoice link via WhatsApp. The tab is opened before the
  // awaits so popup blockers still see the user gesture.
  const handleWhatsAppShare = async (bill: Bill) => {
    const win = window.open("", "_blank");
    try {
      const res = await fetch(`/api/bills/${bill.id}/share`, { method: "POST" });
      if (!res.ok) throw new Error("Failed to create share link");
      const { url } = await res.json();
      const firmName = await getFirmName();
      const message = `Invoice ${bill.billNumber} from ${firmName || "us"} for ₹${parseFloat(bill.total.toString()).toFixed(2)}\n${url}`;
                            ...
                            {openMenuId === bill.id && (
                              ...
                              >
                                <div className="py-1" role="none">
                                  ...
                                  <button
                                    onClick={() => { handleCopyShareLink(bill.id); setOpenMenuId(null); }}
                                  ...
                                  <button
                                    onClick={() => { handleWhatsAppShare(bill); setOpenMenuId(null); }}
                                    className="flex items-center w-full px-4 py-2 text-sm text-gray-700 hover:bg-green-50 hover:text-green-700 transition-colors"
                                    role="menuitem"
                                  >
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      >
                              ...
                              <button
                                onClick={() => { handleCopyShareLink(bill.id); setOpenMenuId(null); }}
                                className="flex items-center w-full px-3 py-2.5 sm:py-2 text-sm text-gray-700 hover:bg-green-50 hover:text-green-700 transition-colors touch-manipulation"
                                role="menuitem"
                              >
                                <svg className="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" />
